...
1. [打开程序：入口如何找到项目](#1-打开程序入口如何找到项目)
2. [启动后台：环境检查与命令传递](#2-启动后台环境检查与命令传递)
3. [统一编排：决定本次启动哪些进程](#3-统一编排决定本次启动哪些进程)
4. [场景出现：车辆与传感器如何加载](#4-场景出现车辆与传感器如何加载)
5. [处理图像：引导式 YOLO 如何生成结果](#5-处理图像引导式-yolo-如何生成结果)
6. [平台就绪：画面记录与报告如何显示](#6-平台就绪画面记录与报告如何显示)
7. [开始巡检：移动停车与视点切换](#7-开始巡检移动停车与视点切换)
8. [结束运行：停止退出与再次打开](#8-结束运行停止退出与再次打开)
9. [切换常规模式：六路检测与点云定位](#9-切换常规模式六路检测与点云定位)
10. [打开岔轨平台：独立导航流程](#10-打开岔轨平台独立导航流程)
11. [打开建图流程：里程计与地图保存](#11-打开建图流程里程计与地图保存)
12. [源码索引与按步骤排障](#12-源码索引与按步骤排障)
